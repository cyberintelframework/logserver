...
###############################################
# Changelog:
# 005 Added upgrade stuff 2.00.02 -> 2.00.03
# 004 Fixed removal of obsolete files
# 003 Fixed upgrade option
# 002 Removed single sensor setup stuff
# 001 version 2.00
###############################################
...
if (-e "$targetdir/webinterface/") {
  printmsg("SURFids logging server already installed:", "info");
  $confirm = "none";
...
if ($confirm =~ /^(install)$/) {
  ...
  while ($e != 0) {
    if ($dbhost != "localhost") {
      printmsg("Creating SURFids tables:", "info");
    ...
    printmsg("Creating SURFids tables:", $?);
    if ($? != 0) { $err++; }
    $e = $?;
    if ($? != 0) {
      $confirm = "a";
  ...
  $confirm = "a";
  while ($confirm !~ /^(1\.03|1\.04|2\.00|skip)$/) {
    $confirm = &prompt("Upgrade database from which version [1.03/1.04/2.00/skip]?: ");
  }

  if ($confirm =~ /^(1\.03|1\.04|2\.00)$/) {
    if ($confirm eq "1.03") {
      $e = 1;
      while ($e != 0) {
        if ($dbhost != "localhost") {
      ...
      while ($e != 0) {
        if ($dbhost != "localhost") {
          `psql -h $dbhost -p $dbport -q -f $targetdir/sql/changes200-202.sql -U "$dbuser" -W "$dbname" 2>>$logfile`;
        } else {
          `sudo -u postgres psql -q -f $targetdir/sql/changes200-202.sql "$dbname" 2>>$logfile`;
        }
        printmsg("Upgrading the database from 2.00 (or 2.00.01) to 2.00.02:", $?);
        if ($? != 0) { $err++; }
        $e = $?;
        if ($? != 0) {
          $confirm = "a";
      ...
      while ($e != 0) {
        if ($dbhost != "localhost") {
          `psql -h $dbhost -p $dbport -q -f $targetdir/sql/changes202-203.sql -U "$dbuser" -W "$dbname" 2>>$logfile`;
        } else {
          `sudo -u postgres psql -q -f $targetdir/sql/changes202-203.sql "$dbname" 2>>$logfile`;
        }
        printmsg("Upgrading the database from 2.00.02 to 2.00.03:", $?);
        if ($? != 0) { $err++; }
        $e = $?;
        if ($? != 0) {
          $confirm = "a";
