...
#############################################
# Changelog:
# 004 added inet_ownsource 
# 003 Fixed bug "could not download" in IE 
# 002 Fixed bugs: display criterea, source address  
# 001 Initial release (split from logsearch.php)
#############################################
...
$pdf->ezText($bigspace . 'Generated at ' . date("d-m-Y H:i:s") . ' by SURFids webinterface', 10);
$pdf->ezText('    ', 30);
...
if ($f_sourcechoice == 3 && !isset($ownsource)) {
  $pdf->ezText("$smaspace All Own Ranges", 10);
}
if (isset($source_ip)) {
  $src_txt = "IP address: $source_ip";
} elseif (isset($ownsource)) {
  $src_txt = "Own Range: $ownsource";
...
#  $ar["ID"] = $id;
  ...
  if ($sevtype == 10 || $sevtype == 11) $ar["Source"] = $srcmac; 
