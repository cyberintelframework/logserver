...
$search = trim(pg_escape_string($_GET['f_search']));
$where = array();
...
if (in_array($_GET['f_reptype'], $valid_reptype)) $rapport = $_GET['f_reptype'];
else $rapport = "multi";
...
$ts_end = trim(pg_escape_string($_GET["ts_end"]));
if (isset($_GET["f_sev"])) {
...
if ($rapport == "idmef") {
    ...
    while ($row = pg_fetch_assoc($result)) {
      ...
      $dest = $row['dest'];
      $dport = $row['dport'];
      $sev = pg_escape_string($row['severity']);
      ...
      if ($numrows_details != 0) {
        ...
        elseif ($sev == 32) {
          ...
          $dia_result_ar = pg_select($pgconn, 'details', $dia_ar);
          $bin = pg_escape_string($dia_result_ar[0]['text']);
...
if ($rapport == "chart_sensor") {
	...
	$f_chart_of = pg_escape_string($_GET["f_chart_of"]);
...
$order_by_tbl = array(	"id"		=> "attacks.id", 
						...
						"keyname"	=> "sensors.id");
if (isset($_GET["order"])) {
	$sql_order_by = pg_escape_string($order_by_tbl[$_GET["order"]]);
}
    ...
    if ($numrows_details != 0) {
      ...
      elseif ($sev == 32) {
        ...
        if ($numrows_bin != 0) {
          $info = $row_bin['info'];
          if ($info == "Suspicious") {
            echo "<td class='datatd'><a href='binaryhist.php?bin=$bin'>$info</a></td>\n";
          }
          else {
            echo "<td class='datatd'><a href='binaryhist.php?bin=$bin'>$info</a></td>\n";
          }
          ...
          echo "<td class='datatd'>Suspicious</td>\n";
        }
      }
      else {
