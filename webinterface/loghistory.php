...
#############################################
# Changelog:
# 1.02.04 Fixed a bug in the virus total count
  ...
  echo "<form name='selectorg' method='get' action='loghistory.php'>\n";
	...
	if (pg_num_rows($query) == 0) {
		...
		if ($_GET["full"] == "malicious") {
			$show = "full list";
			$limit = "";
			$link = "<a href=\"loghistory.php?m=$month&y=$year&org=$s_org\">Show top 5</a><br /><br />\n";
		} else {
			$show = "top 5";
			$limit = "LIMIT 5";
			$link = "<a href=\"loghistory.php?m=$month&y=$year&org=$s_org&full=malicious\">Show full list</a><br /><br />\n";
		}
		echo "<table class='datatable'>\n";
		echo " <tr>\n";
		...
		if ($_GET["full"] == "viruses") {
			...
			$show = "top 5";
			$limit = "LIMIT 5";
			$link = "<a href=\"loghistory.php?m=$month&y=$year&org=$s_org&full=viruses\">Show full list</a><br /><br />\n";
		}
		echo "<table class='datatable'>\n";
		echo " <tr>\n";
		...
		$sql_count = "SELECT SUM(count) FROM stats_history_virus AS shv, stats_virus AS sv WHERE shv.virusid = sv.id AND sv.name <> 'Suspicious' AND shv.historyid = '" . $history_id . "'";
